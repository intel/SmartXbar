...
*/
/*
 * @file  IasAlsaHandlerTestPlaybackStartStop.cpp
 *
 * @date  2015
 */
...
TEST_F(IasAlsaHandlerTest, streamPlaybackStereoStartStop)
{
  std::cout << "=================================================================" << std::endl;
  std::cout << "Test of the ALSA handler in playback direction - using start/stop" << std::endl;
  std::cout << "=================================================================" << std::endl;
  ...
  *devParams = cSinkDeviceParams;
  ...
  while (cntPeriods < numPeriodsToProcess)
  {
    ...
    if ((cntPeriods % 10) == 0)
    {
      std::cout << "stopping playback for 200 ms, then starting again..." << std::endl;
      myAlsaHandler->stop();
      usleep(200000);
      result = myAlsaHandler->start();
      ASSERT_EQ(IasAlsaHandler::eIasOk, result);
