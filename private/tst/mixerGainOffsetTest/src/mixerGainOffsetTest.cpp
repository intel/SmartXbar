...
void IasMixerSetBalanceTest::setInputGainOffset(const char* pin, int32_t gain)
{
...
TEST_F(IasMixerSetBalanceTest, main_test)
{
  ...
  do
  {
    ...
    {
      ioFrame[0][i] = ((*tempData1++)*0.1f);
      ioFrame[1][i] = ((*tempData1++)*0.1f);
      ioFrame[2][i] = ((*tempData2++)*0.1f);
      ioFrame[3][i] = ((*tempData2++)*0.1f);
      ioFrame[4][i] = ((*tempData3++)*0.1f);
      ioFrame[5][i] = ((*tempData3++)*0.1f);
    }
    ...
    const int32_t gain = 60;   // 60[dB/10] => 6dB

    switch (frameCounter)
    {
      case 1000:
        printf("after %d frames: ", frameCounter);
        printf("set input gain offset of stream 0 to 2.0\n");
        setInputGainOffset("Sink0", gain);
        break;

      case 4000:
        printf("after %d frames: ", frameCounter);
        printf("set input gain offset of stream 1 to 2.0\n");
        setInputGainOffset("Sink1", gain);
        break;

      case 7000:
        printf("after %d frames: ", frameCounter);
        printf("set input gain offset of stream 2 to 2.0\n");
        setInputGainOffset("Sink2", gain);
        break;

      case 10000:
        printf("after %d frames: ", frameCounter);
        printf("set i+nput gain offset of stream 0 to 0.5\n");
        setInputGainOffset("Sink0", -gain);
        break;

      case 13000:
        printf("after %d frames: ", frameCounter);
        printf("set input gain offset of stream 1 to 0.5\n");
        setInputGainOffset("Sink1", -gain);
        break;

      case 16000:
        printf("after %d frames: ", frameCounter);
        printf("set input gain offset of stream 2 to 0.5\n");
        setInputGainOffset("Sink2", -gain);
        break;

      case 19000:
        printf("after %d frames: ", frameCounter);
        printf("set input gain offset of all streams to 1.0\n");
        setInputGainOffset("Sink0", 0);
        setInputGainOffset("Sink1", 0);
        setInputGainOffset("Sink2", 0);
        break;
    }
