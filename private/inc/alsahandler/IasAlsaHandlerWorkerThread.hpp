 ...
 * @brief
 */
class IAS_AUDIO_PUBLIC IasAlsaHandlerWorkerThread : public IasIRunnable
{
  public:
    ...
    struct IasAlsaHandlerWorkerThreadParams
    {
      IasAlsaHandlerWorkerThreadParams()
        ...
        ,asrcBufferParams()
      {}
      IasAlsaHandlerWorkerThreadParams(std::string          p_name,
      ...
      {}
      std::string          name;               // Name of the ALSA device
  ...
  private:
     ...
     * @param[in]  asrcBufferOffset                  Offset of the ASRC buffe
    ...
    IasDiagnosticStreamPtr               mDiagnosticStream;        //!< Create AlsaHandler/ASRC diagnostics if configured
    uint32_t                             mLogCntDev;               //!< Log counter to control the amount of log messages related to the ALSA device
    uint32_t                             mLogCntDevTotal;          //!< Total log count
};
